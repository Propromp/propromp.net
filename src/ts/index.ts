...
function placeDusts(): { noise: SimplexNoise, mesh: Mesh }[] {
    const dusts: { noise: SimplexNoise, mesh: Mesh }[] = []
    for (let i = 0; i < 100; i++) {
...
function getContactElement(text: string, font: Font, material: Material, callback: () => void): Group {
...
function getTextGeometry(text: string, font: Font, scale: number, height: number) {
